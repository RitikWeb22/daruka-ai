import { EnvironmentalState, CausalLink } from './schema';

export interface EcologicalRule {
  id: string;
  name: string;
  condition: (state: EnvironmentalState) => boolean;
  involved_variables: string[];
  chain: CausalLink;
  recommendation_hints: string[];
}

export const ECOLOGICAL_RELATIONSHIPS: EcologicalRule[] = [
  {
...
export function extractCausalChains(state: EnvironmentalState): CausalLink[] {
  const chains: CausalLink[] = [];
  for (const rule of ECOLOGICAL_RELATIONSHIPS) {
