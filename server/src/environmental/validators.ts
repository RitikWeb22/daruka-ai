import { EnvironmentalState } from './schema';
...
export const CRITICAL_VARIABLE_DEFINITIONS = [
...
export function calculateCompleteness(state: EnvironmentalState, threshold = 0.5): CompletenessResult {
...
export function parseTextForEnvironmentalState(text: string): Partial<EnvironmentalState> {
  ...
  // Crop & Land Use
  if (lower.includes('wheat')) patch.land_use!.crop = 'wheat';
  if (lower.includes('corn') || lower.includes('maize')) patch.land_use!.crop = 'corn';
  if (lower.includes('soy') || lower.includes('soybean')) patch.land_use!.crop = 'soybean';
  if (lower.includes('rice')) patch.land_use!.crop = 'rice';
  if (lower.includes('barley')) patch.land_use!.crop = 'barley';
  if (lower.includes('cotton')) patch.land_use!.crop = 'cotton';
